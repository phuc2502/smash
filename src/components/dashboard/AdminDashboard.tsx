...
export default function AdminDashboard() {
  ...
  const stats: StatCard[] = [
    ...
    },
    {
      ...
      trend: "+3",
      up: true,
      subtitle: "giáo viên mới",
      color: "mint",
      icon: GraduationCap,
      to: "/users?role=Giáo viên",
    },
    {
      label: "Lớp học Đang diễn ra",
      value: globalStats.activeClasses.toLocaleString(),
      trend: "",
      up: null,
      subtitle: "Phân bổ trên các khối",
      color: "mint",
      icon: CalendarDays,
      to: "/classes?status=Đang diễn ra",
    },
    {
      label: "Bài tập Chờ chấm",
      value: globalStats.pendingGrading.toLocaleString(),
      trend: "-0.5%",
      up: false,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <motion.button
            ...
            aria-label={`${stat.label}: ${stat.value}. Mở ${stat.subtitle}`}
          >
        ...
        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col">
          ...
          <div className="flex-1 space-y-8">
            {recentActivities.length > 0 ? recentActivities.map((activity, i) => (
              ...
              <p className="text-sm text-slate-400">Chưa có dữ liệu hoạt động</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col">
          ...
          <div className="space-y-4">
            ...
            {pendingTasks.length === 0 && (
              <div className="text-center py-8 text-slate-400 text-sm">Tất cả bài tập đã được chấm điểm!</div>
            )}
          </div>
        </div>
      </div>
