...
import {
  ...
  Clock, Search, UserCheck, Inbox, User
} from "lucide-react";
import { motion, AnimatePresence } from "motion/react";
import { useAppContext, LateRequest, ROLE_LABELS } from "../context/AppContext";
...
export default function LateRequestsPage() {
  ...
  const safeLateRequests = lateRequests ?? [];
  ...
  const [editingRequest, setEditingRequest] = useState<LateRequest | null>(null);
  ...
  // Filter late requests based on user role and filters
  const filteredLateRequests = useMemo(() => {
    let list = [...safeLateRequests].filter(Boolean);
  ...
  }, [safeLateRequests, currentAccount, isStudent, isParent, isTeacher, parentChildIds, teacherClassIds, statusFilter, searchQuery]);

  // Handle late request submission
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!lateDate) {
      setFormError("Vui lòng chọn ngày đi học muộn.");
      return;
    }
    if (!lateReason.trim()) {
      setFormError("Vui lòng nhập lý do đi học muộn.");
      return;
    }
    ...
    if (editingRequest) {
      ...
      setFormSuccess("Cập nhật đơn đi học muộn thành công!");
      ...
      setFormSuccess("Đơn yêu cầu đi học muộn của bạn đã được gửi thành công!");
    }
  ...
  React.useEffect(() => {
    if (editingRequest) {
      ...
      setLateDate(editingRequest.date);
      setLateReason(editingRequest.reason);
      setFormError("");
      setFormSuccess("");
    } else {
      setLateReason("");
  ...
  const handleResolve = (requestId: string, status: "approved" | "rejected") => {
    ...
    resolveLateRequest(requestId, status, currentAccount.name);
  };
  ...
  const getStatusBadge = (status: LateRequest["status"]) => {
    switch (status) {
      case "approved":
        return (
    ...
    <div className="space-y-10 pb-10">
      ...
      <div>
        ...
        <p className="text-slate-500 font-medium mt-2 max-w-2xl">
          {isStudent || isParent
            ? "Gửi đơn xin đi học muộn đến ban quản trị trung tâm và theo dõi trạng thái xử lý."
            : "Xem, theo dõi và phê duyệt đơn xin đi học muộn từ học viên."}
        </p>
      </div>
          ...
          <div className="xl:col-span-1 bg-white/80 backdrop-blur-xl p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                >
                  <option value="">-- Chọn lớp học xin đi muộn --</option>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Ngày đi muộn</label>
                <input
                  type="date"
                  value={lateDate}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Lý do đi muộn</label>
                <textarea
                  value={lateReason}
                  onChange={e => setLateReason(e.target.value)}
                  rows={4}
                  placeholder="Vui lòng nêu lý do đi muộn cụ thể (ví dụ: hỏng xe, bận việc trường học chính khóa...)"
              ...
              <div className="flex gap-3 w-full">
                ...
                >
                  {editingRequest ? "Lưu thay đổi" : "Gửi yêu cầu đi muộn"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className={`bg-white/80 backdrop-blur-xl p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-6 ${
          ...
          <div className="flex gap-2 border-b border-slate-100 pb-4 overflow-x-auto">
            {(["all", "pending", "approved", "rejected"] as const).map(f => {
              const count = safeLateRequests.filter(r => {
                if (!r) return false;
            ...
            {filteredLateRequests.length === 0 ? (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-medium">Hệ thống chưa ghi nhận đơn đi muộn nào trùng khớp với bộ lọc.</p>
                </div>
              </motion.div>
            ) : (
              <motion.div
                ...
                className="space-y-4"
              >
                {filteredLateRequests.map((request) => (
                  <motion.div
                    key={request.id}
                    layout
                  ...
                  >
                    <div className="space-y-3 flex-1">
                      ...
                      <div className="flex items-center gap-1.5 text-sm font-black text-slate-950">
                        ...
                        <span>Đi muộn ngày: {safeFormatDate(request.date)}</span>
                      </div>

                      {/* Reason */}
                      <div className="text-xs text-slate-600 leading-relaxed font-medium bg-white/70 p-3 rounded-xl border border-slate-100/50">
                        <p className="font-bold text-slate-400 uppercase tracking-widest text-[9px] mb-1">Lý do đi học muộn</p>
                        {request.reason}
                      </div>
                    ...
                    <div className="flex flex-col items-start md:items-end justify-between self-stretch shrink-0">
                      ...
                      {request.status === "pending" && (
                        ...
                        <div className="flex items-center gap-2 mt-4 md:mt-0">
                          ...
                          <button
                            onClick={() => {
                              if (window.confirm("Bạn có chắc chắn muốn xóa đơn xin đi muộn này không?")) {
                                deleteLateRequest(request.id);
                      ...
                      {request.status === "approved" && (
                        <div className="hidden md:flex items-center gap-1 text-[10px] font-bold text-mint-600 uppercase tracking-widest mt-2">
                          <span>Đồng bộ điểm danh đi muộn có phép</span>
                        </div>
                      )}
                    </div>
                  </motion.div>
