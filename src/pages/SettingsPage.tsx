...
const ALL_PERMISSIONS: { key: PermissionKey; label: string }[] = [
  { key: 'view_dashboard', label: 'Xem trang chủ' },
  { key: 'manage_users', label: 'Quản lý người dùng' },
  { key: 'manage_devices', label: 'Quản lý thiết bị' },
  { key: 'view_access_logs', label: 'Xem nhật ký truy cập' },
  { key: 'manage_classes', label: 'Quản lý lớp học' },
  { key: 'view_classes', label: 'Xem lớp học' },
  { key: 'manage_materials', label: 'Quản lý tài liệu' },
  { key: 'view_materials', label: 'Xem tài liệu' },
  { key: 'manage_assignments', label: 'Quản lý bài tập' },
  { key: 'view_assignments', label: 'Xem bài tập' },
  { key: 'submit_assignment', label: 'Nộp bài tập' },
  { key: 'grade_assignments', label: 'Chấm điểm' },
  { key: 'view_own_grades', label: 'Xem điểm cá nhân' },
  { key: 'export_grades', label: 'Xuất bảng điểm' },
  { key: 'manage_security', label: 'Quản lý bảo mật' },
  { key: 'manage_attendance', label: 'Quản lý điểm danh' },
  { key: 'view_attendance', label: 'Xem điểm danh' },
];
...
export default function SettingsPage() {
  ...
  const [permFeedback, setPermFeedback] = useState<{ tone: 'success' | 'error'; message: string } | null>(null);
  ...
  const handleSelectUser = (userId: string) => {
    ...
    setSelectedPermissions(accountPermissionOverrides[userId] ?? []);
  ...
  const handleSavePermissions = () => {
    ...
    updateAccountPermissions(selectedUserId, selectedPermissions);
    const user = users.find(u => u.id === selectedUserId);
    setPermFeedback({ tone: 'success', message: `Đã cập nhật phân quyền cho ${user?.name ?? selectedUserId}.` });
    setTimeout(() => setPermFeedback(null), 3000);
  };

  const handleSavePassword = () => {    if (!passwordForm.currentPassword || !passwordForm.nextPassword) {
          ...
          {canManageUsers && (
            ...
            >
              <div className="p-6 border-b border-slate-50 flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-slate-900">Phân quyền tài khoản</h3>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Cấp quyền bổ sung</p>
                </div>
              </div>
              <div className="p-5 space-y-4">
                {/* User selector */}
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Chọn tài khoản</label>
                  <div className="relative">
                    <select
                      value={selectedUserId}
                      onChange={e => handleSelectUser(e.target.value)}
                      className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-3 px-4 text-sm font-bold focus:ring-4 focus:ring-violet-500/10 focus:border-violet-500/50 outline-none transition-all appearance-none"
                    >
                ...
                {selectedUserId && (
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Quyền bổ sung</label>
                    <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                      {ALL_PERMISSIONS.map(({ key, label }) => (
                        <label key={key} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-50 cursor-pointer group">
                          <input
                            type="checkbox"
                            checked={selectedPermissions.includes(key)}
                            onChange={() => togglePermission(key)}
                            className="w-4 h-4 rounded text-violet-600 border-slate-300 focus:ring-violet-500/20"
                          />
                          <span className="text-xs font-semibold text-slate-700 group-hover:text-violet-700 transition-colors">{label}</span>
                        </label>
                      ))}
                    </div>
                    ...
                    {permFeedback && (
                      <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold ${
                        permFeedback.tone === 'success' ? 'text-mint-700 bg-mint-50' : 'text-rose-700 bg-rose-50'
                      }`}>
                    ...
                    <button
                      ...
                      className="w-full py-3 bg-violet-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-violet-700 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-violet-100"
                    >
